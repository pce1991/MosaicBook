How this documentation works: Function names are listed followed by
their arguments. If there are multiple overloaded versions of a function
all versions will be listed.
...
Returns true on the frame where the given InputID for device is pressed,
...
Returns true if the given InputID for device is held down, otherwise
returns false.
...
Returns true if the given InputID for device has been held down for at
least N seconds, otherwise returns false.
...
Returns true if the given InputID for device was released this frame,
otherwise returns false.
...
Sets the tiles starting at the x & y coordinate and moving to the right
by width and down by height.
...
=== int32 GetMouseX()
<int32-getmousex>
Returns the x coordinate of the tile the mouse is hovered over. If the
mouse is off the grid returns -1.

=== int32 GetMouseY()
<int32-getmousey>
Returns the y coordinate of the tile the mouse is hovered over. If the
mouse is off the grid returns -1.

=== DrawTextTop(vec4 color, real32 scale, const char \*fmt, …)
<drawtexttopvec4-color-real32-scale-const-char-fmt>
For example:
...
<drawtexttilevec2-position-real32-scale-vec4-color-const-char-fmt>
For example:
...
<drawtextscreenvec2-position-real32-scale-vec4-color-bool-center-const-char-fmt>
...
Evaluates to a vec2 with specified x and y values. If only one value is
passed in sets both x and y to that value.
...
<minreal32-n-real32-m>
Returns the minimum value of n and m.

=== Max(real32 n, real32 m)
<maxreal32-n-real32-m>
Returns the maximum value of n and m.

=== Lerp(real32 a, real32 b, real32 t)
<lerpreal32-a-real32-b-real32-t>
Returns the interpolated value between a and b for t. Note that t is not
clamped between 0 and 1, so any values outside that range can return a
value that is not between a and b. There are also versions of this
...
<bool-raycastaabbvec2-min-vec2-max-vec2-orig-vec2-dir-real32-t>
Tests the ray described by orig and dir against the boundix box of min
and max. Returns true if there is an intersection. It will write out
into the t value. Note that dir is expected to be normalized.
...
=== DeltaTime
<deltatime>
Gives the amount of time in seconds between the start of the last frame
