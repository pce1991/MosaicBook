...
Mosaic has been doing is drawing a grid of colored "quads", and we've
been manipulating those colors by calling `SetTileColor`. That grid has
been centered in the screen and the position of these tiles has been
fixed.

To step outside of Mosaic you'll go into `game_code.cpp` and change
`#define MOSAIC 1` to `#define MOSAIC 0`. This will skip compiling
the`my_mosaic.cpp` file and compile `my_game.cpp` instead. This has a
familiar structure where we have an update and an init function.

Outside the Mosaic layer we have the ability to simply render a colored
rectangle of any size and any position. This is done using the
`DrawRect` function.
