== RANDOM VALUES
<random-values>
To generate a random integer value you can use `RandiRange()` which
takes two arguments, a min and max value.
...
Let's say you want to generate a vec2 pointing in a random direction. If
you did this:
...
You have a couple problems. First, it's not normalized, but second, it's
possible that that the vector has no magnitude if you generate a value
of 0 for both.
