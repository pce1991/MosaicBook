...
position is 0, and our current x position is 5, if there is an object at
2 we will "tunnel" thru the object because we were never on a position
either at the beginning or end of the frame where our position
overlapped the object.

There are several ways to solve this problem, but the simplest (tho most
costly), is to break our movement down into segments. So if we know that
our minimum width of objects is 1, we can make sure that our object
never moves more than 1 unit without checking for a collision. We of
